...
// dnelson may.2012
// tree.c based on ngb.c (Arepo) and CalcHsml (for Python) by Mark Vogelsberger
//
...
float ngb_treefind (float xyz[3], float hguess, float *val_out)
{
  int iter, n, numngb_int;
  float numngb;
  float fval = 0.0; // total of values*weights
  float ftot = 0.0; // total of values
  float wtsum = 0.0; // total of weights
  ...
  /* found a good number of neighbors, calculate mean function value over the Ngblist inbox */
  for(n = 0; n < numngb_int; n++)
  {
    fval += P[Ngblist[n]].Value * P[Ngblist[n]].Weight;
    ftot += P[Ngblist[n]].Value;
    wtsum += P[Ngblist[n]].Weight;
  }
  ...
  //  printf("  numngb_int=%d DesNumNgb=%d Value[0]=%g\n",numngb_int,DesNumNgb,P[Ngblist[0]].Value);
    ...
    if (TophatMode == 1) {
      ...
      *val_out = fval / wtsum;
      ...
      /* total */ // makes sense for mass, total linear/angular momentum
      *val_out = ftot;
    } else if (TophatMode == 3) {
      /* density = total/volume */
#ifdef   ONEDIMS
      *val_out = ftot * 0.5 / hguess;
#else
#ifndef  TWODIMS
      *val_out = ftot * PI_INV * 0.75 / (hguess*hguess*hguess);
...
#else
      *val_out = ftot * PI_INV / (hguess*hguess);
#endif
#endif /* ONEDIMS */
    }
...
float ngb_treefind_variable (float searchcenter[3], float hguess, int *numngb_int)
{
  int numngb, no, p;
