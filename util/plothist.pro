PRO plothist, arr, xhist,yhist, BIN=bin,  NOPLOT=NoPlot, $
                 ...
                 rotate = rotate, $
                 weight=weight, fraction=fraction, normfac=normfac ;DNELSON
;+
; NAME:
;      PLOTHIST
; PURPOSE:
...
; desired (default for integer data)     
 ...
 if keyword_set(weight) then $
  yhist = hist1d(y,weight)

 ;show fraction instead (DNELSON)
 if keyword_set(fraction) then $
  yhist /= float(n_elements(y))
