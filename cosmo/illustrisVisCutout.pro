...
pro makeHDF5SnapFromCutout
  ...
  saveFileHdf5 = sP.derivPath + 'cutouts/cutout.' + sP.savPrefix + str(sP.res) + $
    '.h' + str(gcInd) + '.sf' + str(fix(sizeFac*10)) + '_' + str(sP.snap) + '.hdf5'
...
function illustrisVisCutout, sP=sP, gcInd=gcInd, sizeFac=sizeFac, quantName=quantName
  ...
  saveFilename = saveFileBase + 'inds.sav'
  if ~file_test(saveFilename) then begin
    ...
    saveFilename = saveFileBase + 'pos.sav'
  ...
  ; vrad
  saveFilename = saveFileBase + 'vrad.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; vel (xyz)
  saveFilename = saveFileBase + 'vel.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; temp
  saveFilename = saveFileBase + 'temp.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; nelec
  saveFilename = saveFileBase + 'nelec.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; metallicity
  saveFilename = saveFileBase + 'metal.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; xray
  saveFilename = saveFileBase + 'xray.sav'
  if ~file_test(saveFilename) then begin
    ...
    quant *= loadSnapshotSubset(sP=sP,partType='gas',field='dens',inds=inds) ; rho * T^(1/2)
  ...
  ; SZ y-parameter
  saveFilename = saveFileBase + 'sz_y.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; entropy
  saveFilename = saveFileBase + 'entropy.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; volumes (hsml)
  saveFilename = saveFileBase + 'hsml.sav'
  if ~file_test(saveFilename) then begin
  ...
  ; mass
  saveFilename = saveFileBase + 'mass.sav'
  if ~file_test(saveFilename) then begin
...
function illustrisSliceCutout, sP=sP, gcInd=gcInd, sliceWidth=sliceWidth, axes=axes, quantName=quantName
  ...
  if ~file_test(saveFilename) then begin
    ...
    print,'WARNING: NOTE CENTER, CHANGE?'
  ...
  if ~file_test(saveFilename) then begin
    quant = loadSnapshotSubset(sP=sP,partType='gas',field='vel',inds=inds,/verbose)
