...
/*
	      if(P[i].BH_NumNgb < (desnumngb - All.MaxNumNgbDeviation) ||
		 (P[i].BH_NumNgb > (desnumngb + All.MaxNumNgbDeviation)
		  && P[i].BH_Hsml > (1.01 * All.MinGasHsml)))
		{
		  npleft++;
		  ...
		  if(P[i].BH_NumNgb < (desnumngb - All.MaxNumNgbDeviation))
		    Left[i] = DMAX(P[i].BH_Hsml, Left[i]);
		  else
		    {
		      if(Right[i] != 0)
			{
			  if(P[i].BH_Hsml < Right[i])
			    Right[i] = P[i].BH_Hsml;
			}
		      else
			Right[i] = P[i].BH_Hsml;
		  ...
		  if(iter >= MAXITER - 10)
			...
			("i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n   pos=(%g|%g|%g)\n",
			 i, ThisTask, (int) P[i].ID, P[i].BH_Hsml, Left[i], Right[i],
			 (float) P[i].BH_NumNgb, Right[i] - Left[i], P[i].Pos[0], P[i].Pos[1], P[i].Pos[2]);
		      fflush(stdout);
		    }

		  if(Right[i] > 0 && Left[i] > 0)
		    P[i].BH_Hsml = pow(0.5 * (pow(Left[i], 3) + pow(Right[i], 3)), 1.0 / 3);
		  else
		    {
		      if(Right[i] == 0 && Left[i] == 0)
			endrun(8188);	
		  ...
		  if(P[i].BH_Hsml < All.MinGasHsml)
		    P[i].BH_Hsml = All.MinGasHsml;

		  if(Left[i] > All.BlackHoleMaxAccretionRadius)
		    {
		      P[i].BH_Hsml = Left[i] = Right[i] = All.BlackHoleMaxAccretionRadius;
		    }
		}
	      else
		P[i].TimeBin = -P[i].TimeBin - 1;	
