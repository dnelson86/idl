...
int get_group_coordinates(int argc, void *argv[])
{
  ...
  for(i = 0; i < 5; i++)
    printf("ID [%d] = %lld\n", i, ids[i]);
  for(i = GroupLen-1; i > GroupLen-6; i--)
    printf("ID [%d] = %lld\n", i, ids[i]);
...
int get_subhalo_velocities(int argc, void *argv[])
{
  int Ngroups, Nids, TotNgroups, NTask, Num, Nsubhalos, FiNr, GrNr, Nsubs, first_sub;
  float *Subvel;
  //float *Subvel1;
  IDL_STRING *idl_s;
  char *OutputDir;
...
// Here it reads the binary file, seeking the unnecessary data due to S. Hilbert's table
  ...
  //fread(Subvel, sizeof(int), Ngroups*2 + Nsubhalos*3, fd);
  //fread(Subvel1, sizeof(float), Ngroups*6 + 3*Nsubhalos*2, fd);

  fseek(fd, sizeof(int) * Nsubhalos * 3, SEEK_CUR);

  fseek(fd, sizeof(float) * Ngroups *6, SEEK_CUR);

  fseek(fd, sizeof(float) * Nsubhalos * 3, SEEK_CUR);


  fseek(fd, sizeof(float) * first_sub*3, SEEK_CUR);
  fread(Subvel, sizeof(float), Nsubs*3, fd);
  //fseek(fd, sizeof(int) * (Ngroups - GrNr - 1), SEEK_CUR);


  //fread(Subvel, sizeof(float), 3*Nsubhalos, fd);

  //printf("firstsub is : %d \n", first_sub);
  //fread(Subvel, sizeof(int), Nsubhalos*2, fd);
  //fseek(fd, Nsubhalos*sizeof(int) + Ngroups*sizeof(int), SEEK_CUR);
