...
void *_qsort_SD_start (void *args)
{
  struct qsort_args *a = (struct qsort_args *)args;
  _qsort_SD (a->arr, a->left, a->right, a->cmp);
  return NULL;
}
...
inline void swap_SD (void *arr, size_t i, size_t j)
{
  struct sort_data *a = (struct sort_data *)arr + i;
  struct sort_data *b = (struct sort_data *)arr + j;
  struct sort_data tmp;
  tmp = *a;
  *a = *b;
  *b = tmp;  
}
...
void _qsort_SD (void *arr, int left, int right, comparer cmp)
{
  ...
  if (left >= right)
    return;
  swap_SD (arr, left, average2(left, right));
  last = left;
  for (i = left + 1; i <= right; i++) {
    if (cmp(arr + i*sizeof(struct sort_data), arr + left*sizeof(struct sort_data)) < 0) {
      last++;
      swap_SD (arr, last, i);
    }
  }
  swap_SD (arr, left, last);
  if (right-left > THREAD_THRESHOLD) {
    qa = _qsort_args(sizeof(struct sort_data), arr,left,last-1,cmp);
    if (0 == pthread_create(&lt, PTHREAD_CREATE_JOINABLE, _qsort_SD_start, (void*)&qa)) {
      _qsort_SD (arr, last+1, right, cmp);
      pthread_join(lt, NULL);
      return;
    }
  }
  _qsort_SD (arr, left, last-1, cmp);
  _qsort_SD (arr, last+1, right, cmp);
}
